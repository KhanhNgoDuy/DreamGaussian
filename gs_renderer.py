...
class Renderer:
    ...
    def initialize_default_head(self, num_pts=5000, default_head_template=None, radius=0.5):
        # extract vertices `v` from .obj file
        from pywavefront import Wavefront
        from calculate_norm_pos import Mesh

        mesh = Mesh(default_head_template)
        # the template has radius = 0.1, while random initialization has radius = 0.5
        scaled_verts = mesh.verts / 2
        scaled_verts = scaled_verts[:num_pts]
        
#        num_zero_pts = num_pts - scaled_verts.shape[0]
#        temp = np.zeros((num_zero_pts, 3))
#        xyz = np.concatenate((scaled_verts, temp), axis=0)
        
        
        num_rand_pts = num_pts - scaled_verts.shape[0] 

        phis = np.random.random((num_rand_pts,)) * 2 * np.pi
        costheta = np.random.random((num_rand_pts,)) * 2 - 1
        thetas = np.arccos(costheta)
        mu = np.random.random((num_rand_pts,))
        radius = radius * np.cbrt(mu)
        x = radius * np.sin(thetas) * np.cos(phis)
        y = radius * np.sin(thetas) * np.sin(phis)
        z = radius * np.cos(thetas)
        ...
        xyz = np.stack((x, y, z), axis=1)
        xyz = np.concatenate((scaled_verts, xyz), axis=0)
