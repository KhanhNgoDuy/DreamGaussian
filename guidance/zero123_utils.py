...
class Zero123(nn.Module):
    ...
    def get_img_embeds(self, x):
        ...
        v = self.encode_imgs(x.to(self.dtype)) / self.vae.config.scaling_factor     # [1, 4, 32, 32]
        self.embeddings = [c, v]
    ...
    def get_cam_embeddings(self, elevation, azimuth, radius, default_elevation=0):
        ...
        T = torch.from_numpy(T).unsqueeze(1).to(dtype=self.dtype, device=self.device) # [8, 1, 4]       [1, 1, 4]
        return T
