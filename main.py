...
from loss.perceptual.perceptual import PerceptualLoss
from loss.arcface.arcface import IDLoss
...
class GUI:
    def __init__(self, opt):
        ...
        # self.id_loss = IDLoss(self.device)
        self.vgg_face_loss = VGGLoss('loss/models/vgg_face_dag.pth').to(self.device)
    ...
    def prepare_train(self):
        ...
        with torch.no_grad():
            ...
            if self.enable_zero123:
                ...
                # print(f'\n\n\n\n ENABLE_ZERO123 {self.input_img_torch.shape} {self.guidance_zero123.embeddings[0].shape} {self.guidance_zero123.embeddings[1].shape} \n\n\n\n')
                # exit()
    ...
    def train_step(self):
        ...
        for _ in range(self.train_steps):
            ...
            loss = id_loss = perceptual_loss = vgg_face_loss = 0
            ...
            if self.input_img_torch is not None and not self.opt.imagedream:
                ...
                mse_loss = 10000 * (step_ratio if self.opt.warmup_rgb_loss else 1) * F.mse_loss(image, self.input_img_torch)
                l1_loss = 10000 * (step_ratio if self.opt.warmup_rgb_loss else 1) * F.l1_loss(image, self.input_img_torch)
                # perceptual_loss = 10000 * (step_ratio if self.opt.warmup_rgb_loss else 1) * self.perceptual_loss(image, self.input_img_torch)
                # id_loss = 10000 * (step_ratio if self.opt.warmup_rgb_loss else 1) * self.id_loss(image, self.input_img_torch)
                vgg_face_loss = 10000 * (step_ratio if self.opt.warmup_rgb_loss else 1) * self.vgg_face_loss(image, self.input_img_torch)

                # mask loss
                mask = out["alpha"].unsqueeze(0) # [1, 1, H, W] in [0, 1]
                mask_loss = loss + 1000 * (step_ratio if self.opt.warmup_rgb_loss else 1) * F.mse_loss(mask, self.input_mask_torch)
    ...
    def save_model(self, mode='geo', texture_size=1024):
        ...
        if mode == 'geo':
            ...
            glctx = dr.RasterizeCudaContext()       # Self-Added
